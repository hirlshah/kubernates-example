...
class SendEventReminderBeforeTwentyFiveMinuteEmailCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'send:eventRemiderBeforeTwentyFiveMinute';
     ...
     * @var string
     */
    protected $description = 'Event reminder before twenty five minute email send successfully';
    ...
    public function handle()
    {
        ...
        foreach($connections as $connection) {
            $fromTime = Carbon::now();
            $toTime = Carbon::now()->addMinutes(25);
                    ...
                    if(isset($event->reps) && !empty($event->reps)) {
                        foreach($event->reps as $eventRep) { 
                            if($eventRep->pivot->status == ContactBoardStatus::CONFIRMED_FOR_ZOOM) {
                                ...
                                Log::info('Event reminder before twenty five minute email send successfully '.$event->name.' '.$event->id);
                            }
                        }
                    }
                }
