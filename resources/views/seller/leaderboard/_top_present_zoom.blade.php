<div class="leader-thumb-large mb-n3">
    <div class="leader-thumb-bg" style="background-image: url({{ asset('assets/images/leader-bg.svg') }})">
        <div class="leader-thumb-avatar" style="background-image: url({{ $top10present[1]['image'] ?? asset((config('app.rankup.company_default_image_file'))) }})">
        </div>
    </div>
</div>
@if(isset($top10present[1]))
    <div class="text-center mb-2">
        <h6>{{$top10present[1]['name']}}</h6>
    </div>
    <div class="leader-content">
        <div class="d-flex align-items-center justify-content-center mb-1">
            <i class="feather-map-pin blue me-2"></i>
            <span>{{$top10present[1]['location']}}</span>
            @if(isset($top10present[1]['uplineName']))
                <i class="feather-user blue me-2"></i>
                <span>{{$top10present[1]['uplineName']}}</span>
            @endif
        </div>
        <div class="d-flex align-items-center justify-content-center">
            <i class="feather-award blue me-2"></i>
            <span>{{$top10present[1]['count']}}</span>
        </div>
    </div>
@endif
<div class="leader-main-list mt-2">
    @if(count($top10present) > 0)
        @foreach($top10present as $key=>$data)
